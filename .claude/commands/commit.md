...
**scope**: 변경된 주요 모듈명 (예: `user`, `alimtalk`, `reservation-date`)
여러 모듈이 동시에 바뀐 경우 → 커밋 분리를 먼저 제안한다.
...
>
> 1. `feat(memo): DTO 파일 추가` — dto/ 파일들
> 2. `refactor(memo): 서비스에 새 DTO 적용` — service, controller"

## Step 5 — 메시지 초안 제안

형식: `type(scope): (KDS-XXXX) 설명`

- 설명은 한글, 명령형 (`추가`, `수정`, `제거`, `개선`, `리팩토링`)
- 50자 이내 권장
...
```
feat(memo): (KDS-2896) 메모 목록 응답 DTO 및 Mapper 추가
```

## Step 6 — 확인 후 실행

초안을 보여주고 "이 메시지로 커밋할까요?" 확인을 받는다.
수정 요청이 있으면 반영 후 재확인.
